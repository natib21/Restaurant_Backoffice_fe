...
const DINE_IN_STATUS_OPTIONS = [
  ...
  { key: 'pending', label: 'New Table Orders' },
  { key: 'accepted', label: 'Accepted' },
  { key: 'preparing', label: 'In Kitchen' },
  { key: 'ready', label: 'Ready to Serve' },
  { key: 'served', label: 'Served (At Table)' },
  { key: 'completed', label: 'Billed / Closed' },
];

const DineInManagementPage: React.FC = () => {
  ...
  // Socket subscription for real-time dine-in events
  useEffect(() => {
    if (!socket) return;
    ...
    const handleOrderEvent = (payload: any) => {
      ...
      if (orderData.orderType && !['dine_in', 'dine-in'].includes(orderData.orderType?.toLowerCase())) return;
  ...
  // Dine-in KPI stats
  const stats = useMemo(() => {
    const active = dineIns.filter(
      (o) => o.status !== 'canceled' && o.status !== 'cancelled'
    );
    return {
      total: active.length,
      pending: dineIns.filter((o) => o.status === 'pending').length,
      kitchen: dineIns.filter((o) =>
        ['accepted', 'preparing'].includes(o.status)
      ).length,
      readyToServe: dineIns.filter((o) => o.status === 'ready').length,
      served: dineIns.filter((o) => o.status === 'served').length,
      completed: dineIns.filter((o) => o.status === 'completed').length,
  ...
  const statusCounts = useMemo(
    () => ({
      ...
      accepted: dineIns.filter((o) => o.status === 'accepted').length,
      preparing: dineIns.filter((o) => o.status === 'preparing').length,
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl font-black tracking-tight text-foreground flex items-center gap-2">
            ...
            Dine-In Management
            <span className="inline-flex items-center justify-center px-2 py-0.5 text-xs font-mono font-bold bg-emerald-500/10 text-emerald-600 rounded-full">
              {stats.total} Active Tables
            </span>
          </h1>
          <p className="text-xs text-muted-foreground mt-0.5">
            Floor table service, kitchen preparation, and billing operations.
          </p>
        </div>
