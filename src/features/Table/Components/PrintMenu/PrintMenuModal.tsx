// src/features/Table/Components/PrintMenu/PrintMenuModal.tsx
...
import { PremiumALaCarteTemplate } from './templates/PremiumALaCarteTemplate';
import { EthiopianClassicTemplate } from './templates/EthiopianClassicTemplate';
import { downloadMenuAsPdf, triggerCleanMenuPrint } from './utils/printPdfUtils';
import {
...
export const PrintMenuModal: React.FC<PrintMenuModalProps> = ({
  ...
  const [activeTab, setActiveTab] = useState<'template' | 'customize' | 'preview'>('preview');
  ...
  // Keep restaurant name in sync with merchant if not customized
  useEffect(() => {
    if (merchant?.businessName && !settings.restaurantName) {
      setSettings((prev) => ({
        ...prev,
  ...
  useEffect(() => {
    if (initialTable?._id) {
      setSelectedTableId(initialTable._id);
    } else if (allTables.length > 0 && !selectedTableId) {
      setSelectedTableId(allTables[0]._id);
    }
  }, [initialTable, allTables]);

  const activeTable =
    allTables.find((t) => t._id === selectedTableId) || initialTable;

  // Print Menu Trigger (Clean Iframe Print)
  const handlePrint = () => {
    if (!activeTable) {
    ...
    try {
      ...
      toast.success('Opening print dialog for Table ' + activeTable.tableNumber);
    } catch (err: any) {
  ...
  const handleDownloadPdf = async () => {
    ...
    try {
      ...
      toast.success('Menu PDF downloaded successfully!', { id: toastId });
    } catch (err: any) {
  ...
  // Compute paper dimension aspect ratio
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent
        className={`p-0 flex flex-col bg-slate-100 dark:bg-slate-950 border-slate-300 dark:border-slate-800 overflow-hidden shadow-2xl transition-all duration-200 ${
          isFullScreen
            ? 'fixed inset-0 w-screen h-screen max-w-none max-h-none rounded-none border-none z-50'
            : 'max-w-[96vw] xl:max-w-[1400px] h-[92vh] rounded-3xl'
        }`}
      >
        {/* Top App Bar */}
        <header className="px-4 sm:px-6 py-3 bg-white dark:bg-slate-900 border-b border-slate-200 dark:border-slate-800 flex items-center justify-between gap-4 flex-shrink-0 z-10">
          <div className="flex items-center gap-3">
            ...
            <div>
              <DialogTitle className="text-base font-bold text-slate-900 dark:text-white flex items-center gap-2">
                <span>Print Physical Menu</span>
                <Badge variant="outline" className="text-[10px] font-semibold bg-slate-50 dark:bg-slate-800">
                  {settings.templateId === 'ethiopian-classic' ? 'Template 2 — Ethiopian Classic' : 'Template 1 — Premium À La Carte'}
                </Badge>
                <Badge variant="outline" className="text-[10px] font-semibold bg-slate-50 dark:bg-slate-800 hidden sm:inline-flex">
              ...
              <DialogDescription className="text-xs text-slate-500 hidden sm:block">
                Full-page vector print layout and 300-DPI PDF export with embedded table QR code
              </DialogDescription>
            </div>
          </div>

          {/* Quick Controls: Table Switcher & Print / PDF Actions */}
          <div className="flex items-center gap-2">
            ...
            {allTables.length > 0 && (
              <div className="flex items-center gap-1.5 mr-1">
                ...
                <Select value={selectedTableId} onValueChange={setSelectedTableId}>
                  <SelectTrigger className="h-8 text-xs font-bold w-[120px] sm:w-[140px] bg-slate-50 dark:bg-slate-800 border-slate-200 dark:border-slate-700">
            ...
            <Button
              ...
              variant="outline"
              className="h-8 px-3 text-xs font-bold rounded-xl gap-1.5 shadow-2xs border-slate-300 dark:border-slate-700 hover:bg-slate-100 dark:hover:bg-slate-800"
            >
              {isGeneratingPdf ? (
            ...
            <button
              ...
              className="p-1.5 text-slate-400 hover:text-slate-600 dark:hover:text-slate-200 rounded-lg hover:bg-slate-100 dark:hover:bg-slate-800 ml-1"
        ...
        {/* Main Body: Left Configuration Studio + Right Live Preview */}
        <div className="flex-1 flex flex-col lg:flex-row overflow-hidden">
          {/* Left Customization Studio */}
          <div className="w-full lg:w-[360px] xl:w-[400px] bg-white dark:bg-slate-900 border-r border-slate-200 dark:border-slate-800 flex flex-col h-full flex-shrink-0 z-0">
            <Tabs
              value={activeTab}
          ...
          <div className="flex-1 bg-slate-200/70 dark:bg-slate-950/80 p-3 sm:p-6 lg:p-8 flex flex-col items-center justify-start overflow-y-auto relative">
            {/* Zoom & Table Indicator Bar */}
            <div className="w-full max-w-4xl flex items-center justify-between pb-3 text-xs text-slate-500 flex-shrink-0">
              <div className="flex items-center gap-2">
                <span className="font-semibold text-slate-700 dark:text-slate-300">
                  {activeTable ? `Table ${activeTable.tableNumber} Menu Preview` : 'Menu Preview'}
                </span>
                <span className="text-slate-400">•</span>
                <span>{menuItems.length} items loaded from API</span>
              </div>
            ...
            >
              {activeTable ? (
                ...
                <div className="h-64 flex items-center justify-center text-slate-400 text-sm bg-white rounded-2xl border border-slate-200">
                  Please select a table to preview the menu
                </div>
              )}
            </div>
